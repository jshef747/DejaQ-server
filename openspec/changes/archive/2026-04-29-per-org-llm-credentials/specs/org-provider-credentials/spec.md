...
The system SHALL expose `DELETE /admin/v1/orgs/{org_slug}/credentials/{provider}` returning HTTP 200 with `{"deleted": true}` on success. If no credential exists for that provider, the response SHALL be HTTP 404. Unknown org SHALL return HTTP 404. Org inaccessible to the caller SHALL return HTTP 403.
